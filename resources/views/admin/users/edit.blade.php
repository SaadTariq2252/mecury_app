        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-6 p-6">
            ...
            <div>
                ...
                <select name="scoped_path_id" id="scoped_path_id"
                    ...
                    @foreach($scopedPaths as $path)
                        <option value="{{ $path->id }}" {{ old('scoped_path_id', $user->scoped_path_id) == $path->id ? 'selected' : '' }}>
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <p class="text-sm text-blue-800">
                    @if($user->scopedPath)
                        Currently assigned to: <strong>/{{ $user->scopedPath->path_identifier }}</strong> ({{ $user->scopedPath->name }})
                    @else
                        No path currently assigned
                    @endif
                </p>
            </div>
